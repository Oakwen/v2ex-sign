...
const MESSAGER_URL = process.env.MESSAGER_URL;
...
function send_wechat(msg) {

    return new Promise(async (resolve) => {

        try {
            console.log(MESSAGER_URL)

            let res = await axios.post(MESSAGER_URL, `v2ex签到(づ ●─● )づ\n${msg}`)

            if (res.data.errmsg == '数据发送成功') {
